...
export default function EditBookingPage() {
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      try {
        const bookingId = Array.isArray(id) ? parseInt(id[0]) : parseInt(id as string);
        const booking = await crudService.getBooking(bookingId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await crudService.updateBooking(bookingId, bookingData);

      alert(`Booking for "${formData.customerName}" updated successfully!`);
      ...
      <ProtectedRoute>
        <div className="max-w-4xl mx-auto">
          <div className="mb-6">
            ...
            <h1 className="text-3xl font-bold text-white mt-2">Editing Booking</h1>
            <p className="text-gray-400">Loading booking data...</p>
          </div>
  ...
  return (
    <ProtectedRoute>
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <Link href={`/bookings/${id}`} className="text-purple-400 hover:text-purple-300 mb-4 inline-block">
            ← Back to Booking
          </Link>
          <h1 className="text-3xl font-bold text-white mt-2">Edit Booking</h1>
          <p className="text-gray-400">Update booking information</p>
        </div>
