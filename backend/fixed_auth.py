...
async def login(
    ...
    try:
        ...
        access_token = create_access_token(
            data={"sub": user.username, "role": getattr(user.role, 'value', user.role)},
...
async def refresh_token(
    # For simplicity, we'll use a basic refresh mechanism
    # In a real implementation, you'd have a refresh token in the request
    token_data: str = Form(..., alias="refresh_token"),
    db: AsyncSession = Depends(get_db)
):
    """
    Refresh access token using refresh token
    """
    # This is a simplified version - in production you'd validate the refresh token
    # and check it against a stored refresh token

    # For now, just return an error since we don't have the full token refresh mechanism
    raise HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="Refresh token validation not implemented",
    )